 ...
 * @details compile instructions:
 *              gcc -c array.c
 *              gcc -g -Wall array.o lab3.c -o l3
 * @bugs none
 * @todo none
 */
...
int main(void)
{
        ...
        * Lab 3 Test Array
        *
        * Uncomment and test your code with this array once you are done writing
	...
	print_array(a, size);
        ...
        int found_max = 0;
        found_max = find_max(a, size);
        //
        int found_min = 0;
        found_min = find_min(a, size);
        //
        int found_midpoint = 0;
        found_midpoint = midpoint(min, max);
        //
        int LT_midpoint = 0; 
        LT_midpoint = get_count(a, size, LT, mid);
        //
        int GT_midpoint = 0;
        GT_midpoint = get_count(a, size, GT, mid);
        //
        int searchvalue = 0;
        searchvalue = linear_search(a, size, found_midpoint);
        //
        int foundsum = 0;
        foundsum = sum(a, size);
        //
        float foundavg = 0;
        foundavg = average(a, size, foundsum);
        //
        int foundmed = 0; // median
        foundmed = median(a, size);
        //
        int evens = 0;
        ...
        //
        int min_divisibles = 0;
        min_divisibles = divisible_count(a, size, min);
        //


        /*test values*/
        int lte7 = 0;
        lte7 = get_count(a, size, LTE, 7);
        int gte10 = 0;
        gte10 = get_count(a6, size, GTE, 10);
        int ee10 = 0;
        ee10 = get_count(a6, size, EE, 1);

        
        /* you will be updating this function call throughout this lab until it has 13 parameters */
        print_summary(size, found_max, found_min, found_midpoint, GT_midpoint, LT_midpoint, searchvalue, foundsum, foundavg, foundmed, evens, odds, min_divisibles);

        reverse(a, size);
        //print_array(a, size); // prints the newly reversed array
	return 0;
}

/**
 * @brief Prints the result of the array algorithms
 * 
 * @param size array size
 * @param found_max stored array max
 * @param found_min stored array min
 * @param found_midpoint stored array midpoint
 * @param GT_midpoint stored values greater than the midpoint
 * @param LT_midpoint stored values less than the midpoint
 * @param searchvalue found/notfound linear search index (midpoint)
 * @param foundsum stored sum of array
 * @param foundavg stored avg of array
 * @param foundmed stored median of array
 * @param evens stored even count
 * @param odds stored odd count
 * @param min_divisibles stored count of min divisibles
 */
void print_summary(size_t size, int found_max, int found_min, int found_midpoint, int GT_midpoint, int LT_midpoint, int searchvalue, int foundsum, float foundavg, int foundmed, int evens, int odds, int min_divisibles)
{
        ...
        printf("the midpoint of the array = %d\n", found_midpoint);
        printf("the number of values in the array greater than the midpoint = %d\n", GT_midpoint);
        printf("the number of values in the array less than the midpoint = %d\n", LT_midpoint);

        if (searchvalue == -1){
                printf("midpoint not found in list\n");
        }
        else {
                printf("midpoint found in list\n");
        }
        ...
        printf("the count of even values in the array = %d\n", evens);
        printf("the count of odd values in the array = %d\n", odds);
        printf("the count of values divisibly by min value in the array = %d\n", min_divisibles);
}
